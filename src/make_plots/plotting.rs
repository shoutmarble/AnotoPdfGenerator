...
pub fn draw_preview_image(
    ...
    let root_area = BitMapBackend::new(filename, (img_width, img_height)).into_drawing_area();
    root_area.fill(&WHITE)?;

    // Use PDF coordinate system: 0..width, 0..height (points)
    // This assumes PDF origin is bottom-left.
    let mut chart =
        ChartBuilder::on(&root_area).build_cartesian_2d(0f64..a4_width_pts, 0f64..a4_height_pts)?;
...
pub fn draw_dot_on_file(
    ...
    // Convert to pixels
    // The GUI preview uses a Y-flipped matrix by default, so treat (0,0)
    // as the upper-left corner in screen/pixel coordinates.
    let x_px = (x_pos_pt * scale).round() as i32;
    let y_px = (y_pos_pt * scale).round() as i32;
...
pub fn draw_dots_on_file(
    ...
    let (w, h) = img.dimensions();
    let color = image::Rgba([255, 0, 0, 255]);
    let radius = (config.dot_size as f64 * scale * 2.0).max(5.0) as i32;
...
pub fn draw_dots(
    ...
    draw_dots_y_axis(
        bitmatrix,
        _grid_size,
        &format!("output/{}__Y.png", base_filename),
    )?;

    let root_area = BitMapBackend::new(&filename, (800, 400)).into_drawing_area();
    ...
    // Draw circles based on bitmatrix values
    ctx.draw_series((0..bitmatrix.dim().0).flat_map(|y| {
